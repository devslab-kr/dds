...
다크 배경, `#fafafa` 라이트) 을 시스템 기준으로 확정한다.

**단, 시스템은 멀티 브랜드다 (2026-08-13 개정, asklinq D-051).** cyan 은
데브스랩 **코퍼레이트** 브랜드이지 모든 제품의 브랜드가 아니다 — 제품이
회사 홈페이지와 같은 색을 쓰면 제품 정체성이 사라진다는 소유자 결정.
제품은 **자기 브랜드 색을 소유**하고 (AskLinq = teal, 제품 레포의
`brand.ts` 가 명명), 뉴트럴·상태색·규칙·구조는 DDS 를 공유한다. on-brand
규칙 (아래 노트) 은 색이 무엇이든 **모든 채워진 브랜드 면**에 적용된다.
...
- 24px 그리드, `currentColor` 로 색 상속, 라운드 캡·조인.
- **스트로크는 코어(제품) 세트 1.6px, devslab.kr `site-` 세트 1.8px**
  (2026-08-15 개정, D-013 — 원래의 "1.5px"는 에셋이 존재하기 전에 쓴
  숫자다. 실제 출하 세트는 1.6이고, 마케팅 세트는 32–40px로 그려지므로
  더 굵다).

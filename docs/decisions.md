...
**결정.** `@devslab/dds-table`을 새 패키지로 추가한다. TanStack Table을
`@tanstack/solid-table@9.2.4`에 **캐럿이 아니라 정확히** 고정한다 —
dds-table은 compatibility-canary가 이미 검증해 둔 프레임워크 스택 위에
얹히는 컴포넌트이므로, 그 스택의 patch/minor 자유도를 스스로 가질 이유가
없고 캐너리가 한 번도 보지 못한 TanStack 버전이 슬쩍 들어오는 길을
원천 차단한다. 컴포넌트는 `DataTable` 하나와 `Column<T>`/`DataTableLabels`
타입만 내보낸다.

컴포넌트 안에서 확정한 것 넷:
- **정렬은 타입이 정한다.** `sort: "client" | { statedOrder: string }`.
  `"client"`는 테이블이 전체 목록을 메모리에 들고 있다는 뜻이라 정렬 버튼이
  안전하다(재정렬 대상이 곧 표시 대상 전체이므로). `{ statedOrder }`는
  서버가 정렬해 커서로 페이징한 한 페이지만 보여준다는 뜻 — 이때는 `sortBy`가
  있는 컬럼이라도 버튼을 렌더링하지 않는다. 한 페이지만 재정렬하면서 "전체를
  정렬했다"처럼 보이는 버튼은 데이터를 조용히 거짓 보고하는 것이기 때문이다.
  대신 실제 순서를 `statedOrder` 문장으로 테이블 위에 찍는다.
- **페이징은 버튼이 아니라 링크다.** `page.nextHref`는 `<a href>`로
  렌더링된다. 커서 페이징 목록은 결국 쿼리 문자열만 다른 URL이고, 진짜
  링크여야 JavaScript가 꺼져 있어도, 하이드레이션 전에도, 브라우저의 "새 탭
  에서 열기"에서도 그대로 동작한다.
- **컬럼 접기는 JavaScript가 아니라 CSS다.** `fold: true` 컬럼은
  `dds-css`의 미디어 쿼리(`@media (max-width: 56.25rem)`)가 `[data-fold]`를
  숨긴다 — resize observer도, 뷰포트별 재렌더도, 정렬·페이징 상태와 맞춰야
  할 동기화 비용도 없다.
- **어휘는 주입한다, 공유하지 않는다.** `DataTableLabels`에는 선택 필드가
  없고 컴포넌트에는 대체 문자열이 없다 — DDS는 소비자 사용자가 어떤 언어를
  읽는지 알 수 없으므로, 방문자가 읽는 어떤 단어도 컴포넌트가 스스로
  렌더링하지 않는다.

테이블 CSS(`.dds-table*`)는 `dds-table` 패키지가 아니라 `dds-css`
(`packages/dds-css/src/table.css`)에 산다 — 이 레포의 다른 프레임워크
컴포넌트 전부가 시각 언어를 그 자리에 두는 기존 패턴을 그대로 따른 것이고,
CSS만 필요한 소비자가 `dds-table`(과 그 TanStack 의존성) 전체를 끌어올
필요를 없앤다.

...
import { CANARY_SECRET_SENTINELS } from "./secret-sentinels.mjs";
...
async function walk(path) {
  for (const entry of await readdir(path, { withFileTypes: true })) {
    ...
    else if (textExtensions.has(extname(entry.name))) { assertNoLikelySecrets(await readFile(target, "utf8"), target, CANARY_SECRET_SENTINELS); scanned += 1; }
  }
}

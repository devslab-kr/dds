...
| `dist/ionic.css` | `@devslab-kr/dds-tokens/ionic.css` | Ionic 앱 (`--ion-*`를 `--dds-*`에 매핑) |
...
- **로드 순서.** `tokens.css`가 먼저. `theme.css`/`ionic.css`는 그 변수를
  ...
  런타임 테마를 따라갑니다.
- **다크 모드.** `<html>`(또는 임의 서브트리 — 패널을 테마에 고정 가능)에
  `data-theme="dark"`. 미디어 쿼리는 내보내지 않습니다 — 자동 추종 여부는
  devslab.kr 토글과 마찬가지로 소비자의 몫입니다.
